...
{
    int threadsPerBlock = num_threads_small;
    int sharedMemSize = threadsPerBlock * 2 * sizeof(float); // 存储值和索引
    max_index_kernel<<<num, threadsPerBlock, sharedMemSize>>>(index, (float*)x, n);
}

REGISTER_OP_FUNCTION(MaxIndex, CUDA, FLOAT32, cuda_fp32_max_index_exec);